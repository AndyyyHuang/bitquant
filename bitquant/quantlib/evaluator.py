...
@nb.njit
def _start_loop(portfolio_weight_matrix, trade_price_matrix, init_asset_value, taker_fee, volume_precision, min_notional):
    n, m = portfolio_weight_matrix.shape
    res = np.zeros((n, m+1))
    res.fill(np.nan)
    position = np.array([0.0] * m)
    cash = init_asset_value
    for i in range(n):
        present_price = trade_price_matrix[i]
        # update asset_value
        asset_value = np.sum(present_price * position) + cash
        # update the current portfolio weight
        current_portfolio_weight = position * present_price / asset_value
        ...
        orders_v = trade_portfolio_weight * asset_value / present_price
...
class Evaluator:
    ...
    def evaluate(self, portfolio_weight_matrix: np.array, price_matrix: np.array, init_asset_value: float, taker_fee: float, volume_precision: np.array, min_notional: np.array):
        ...
        if not always_delta_neutral:
            raise Exception("Not always delta neutral")
