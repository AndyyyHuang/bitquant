...
# 对输入X的结构约定 X [ DATE_COL, (STOCK_COL,features)]->[DATE_COL,STOCK_COL,features]
STOCK_COL = "symbol"
DATE_COL = "col"
...
def _parallel_evolve_3D(n_programs, parents, X, y, sample_weight, seeds, params):
    ...
    init_function_set = params['init_function_set']
    rolling_window_1 = params['rolling_window_1']
    rolling_window_2 = params['rolling_window_2']
    fee = params['fee']
    ...
    for i in range(n_programs):
        ...
        if sample_weight is None:
            ...
            # curr_sample_weight = np.tile(sample_weight.copy(),(n_stocks,1)).T
            curr_sample_weight = sample_weight.copy()
        oob_sample_weight = np.where(curr_sample_weight > 0, 0, 1)

        # indices, not_indices = program.get_all_indices_df_3D(X=X, n_samples=oob_sample_weight,
        #                                                   max_samples=max_samples,
        #                                                   random_state=random_state)
        # X.set_index("交易日期",inplace=True)

        # curr_sample_weight[not_indices] = 0
        # oob_sample_weight[indices] = 0

        program.raw_fitness_ = program.raw_fitness_3D(X, y, curr_sample_weight, rolling_window_1, rolling_window_2, fee)
        if max_samples < n_dates:
...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        self.metric = metric
        self.rolling_window_1=rolling_window_1
        self.rolling_window_2=rolling_window_2
        self.fee=fee
        ...
        self.init_function_set = init_function_set
    ...
    def fit_3D(self, X, y, feature_names, baseIC=None, sample_weight=None, standard_expression=None,need_parallel = True):
        ...
        # else:
        #     # 判断X是否是df类型且y是否是数值类型
        #     if not isinstance(X, pd.DataFrame) or not pd.to_numeric(y, errors='coerce').notnull().all():
        #         raise ValueError("输入的类型不正确")
        ...
        params['rolling_window_1'] = self.rolling_window_1
        ...
        ##################真正的遗传算法从这里开始#########################################################################
        if baseIC is not None:
            self.total_program = []
        if standard_expression is not None:
            self.focus_program = []
        for gen in range(prior_generations, self.generations):
            ...
            if baseIC is not None:
                for program in population:
                    if program.raw_fitness_ > baseIC:
                        self.total_program.append(program)

            if standard_expression is not None:
                # 判断是样本内，样本外还是都要
                type_dictionary = _syntax_adapter(standard_expression)
                feature_dictionary = {}
                ...
                for program in population:
                    ...
                    # # 字符串替换
                    # for fitness_function_name,fitness_funcion in _fitness_map.items():
                    #     standard_expression.replace(fitness_function_name,"_fitness_map['{}']".format(fitness_function_name))
                    ...
                    if len(re.findall(r'-?\d+\.\d+', formulation)) > 0:
                        ...
                        for number in numbers:
                            new_string = new_string.replace(number,
                                                            "np.tile({}, (X.shape[0], X.shape[2]))".format(number))
                    ...
                    for function_name, _function in all_cal_dictionary.items():
                        formulation = formulation.replace(function_name,
                                                          "all_cal_dictionary['{}']".format(function_name))
                    """
                    for cal_type, cal_functions in type_dictionary.items():
                        condition = True
                        if cal_type == "TRA":
                            # print("TRA")
                            for cal_function in cal_functions:
                                TRA_sample_weight = sample_weight.copy()
                                TRA_sample_weight = np.array(TRA_sample_weight)
                                cal_code = "({},y,TRA_sample_weight, self.rolling_window_1, self.rolling_window_2, self.fee)".format(formulation)
                                for fitness_function_name, fitness_funcion in _fitness_map.items():
                                    cal_function = cal_function.replace(fitness_function_name,
                                                                        "_fitness_map['{}']".format(
                                                                            fitness_function_name) + cal_code)

                                current_result = eval(cal_function)
                                condition = condition and current_result
                                if not condition:
                                    break
                            else:
                                continue
                        ...
                        if cal_type == "OOB":
                            ...
                            for cal_function in cal_functions:
                                ...
                                OOB_sample_weight = np.array(OOB_sample_weight)
                                cal_code = "({},y,OOB_sample_weight, self.rolling_window_1, self.rolling_window_2, self.fee)".format(formulation)
                                for fitness_function_name, fitness_funcion in _fitness_map.items():
                                    cal_function = cal_function.replace(fitness_function_name,
                                                                        "_fitness_map['{}']".format(
                                                                            fitness_function_name) + cal_code)
                        ...
                        if cal_type == "TOT":
                            ...
                            for cal_function in cal_functions:
                                # 先做TRA
                                TRA_function = cal_function
                                TRA_sample_weight = sample_weight.copy()
                                TRA_sample_weight = np.array(TRA_sample_weight)
                                cal_code = "({},y,TRA_sample_weight, self.rolling_window_1, self.rolling_window_2, self.fee)".format(formulation)
                                for fitness_function_name, fitness_funcion in _fitness_map.items():
                                    TRA_function = TRA_function.replace(fitness_function_name,
                                                                        "_fitness_map['{}']".format(
                                                                            fitness_function_name) + cal_code)
                                current_result = eval(TRA_function)
                                condition = condition and current_result
                                if not condition:
                                    break
                                # 再做OOB
                                OOB_function = cal_function
                                OOB_sample_weight = np.where(sample_weight > 0, 0, 1)
                                OOB_sample_weight = np.array(OOB_sample_weight)
                                cal_code = "({},y,OOB_sample_weight, self.rolling_window_1, self.rolling_window_2, self.fee)".format(formulation)
                                for fitness_function_name, fitness_funcion in _fitness_map.items():
                                    OOB_function = OOB_function.replace(fitness_function_name,
                                                                        "_fitness_map['{}']".format(
                                                                            fitness_function_name) + cal_code)
                                current_result = eval(OOB_function)
                                condition = condition and current_result
                                if not condition:
                                    break
            ...
            # print("Remove old programs that didn't make it into the new population.")
            ...
            self.run_details_['average_length'].append(np.nanmean(length))
            self.run_details_['average_fitness'].append(np.nanmean(fitness))
...
class SymbolicTransformer(BaseSymbolic, TransformerMixin):
    ...
    def __init__(self,
        ...
        super(SymbolicTransformer, self).__init__(
            ...
            function_set=function_set,
            metric=metric,
            rolling_window_1=rolling_window_1,
    ...
    def show_program(self, trainX, Y, feature_names, sample_weight=None, baseIC=False, show_tracing=(False, "./")):
        ...
        Args:
            trainX: array [n_dates,features,n_stocks,]
            Y: array [n_dates,returns]
            sample_weight: array [n_dates,1]->[T,T,T,F,F,F]
            feature_names: List [开盘价,收盘价，...,factor_n]
            baseIC: Bool,default =False
            show_tracing:tuple ,default=(False,None) e.g.:(True,"./show_tracing.csv")

        Returns:pd.DataFrame All metric result of your programs
        ...
        if baseIC:
            ...
            for program in self.total_program:
                ...
                for key, value in all_cal_dictionary.items():
                    OOB_IC_cal_code = re.sub(r"\b" + re.escape(key) + r"\b", "all_cal_dictionary['{}']".format(key), OOB_IC_cal_code)
                    OOB_IR_cal_code = re.sub(r"\b" + re.escape(key) + r"\b", "all_cal_dictionary['{}']".format(key), OOB_IR_cal_code)
                    OOB_RankIC_cal_code = re.sub(r"\b" + re.escape(key) + r"\b", "all_cal_dictionary['{}']".format(key), OOB_RankIC_cal_code)
        ...
        if show_tracing[0]:
            tracing_result = pd.DataFrame(
                columns=["表达式", "fitness", "代数", "OOB fitness", "训练集IC", "样本外IC", "训练集IR", "样本外IR",
                         "训练集RankIC", "样本外RankIC"])
            for item in self.focus_program:
                program, trace_generation = item
                formulation = program.__str__()
                # Use regular expressions to find the number in the string
                if len(re.findall(r'-?\d+\.\d+', formulation)) > 0:
                    # Use regular expressions to find all the numbers in the string
    ...
    def transform_3D(self, X):
        ...
        # X_new = np.array([gp.execute(X) for gp in self._best_programs]).T
        X_new = np.array([gp.execute_3D(X) for gp in self._best_programs]).transpose([1, 0, 2])
        return X_new
