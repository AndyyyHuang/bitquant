...
class factorSelector:
    ...
    @classmethod
    def filter_out_high_corr_factor(cls, threshold=0.6, greater_is_better=False):
        """Backforward select"""
        filtered_factor_lis = []
        factor_lis = cls.corr_df.columns.tolist()
        for i in reversed(range(len(cls.corr_df))):
            corr_ser = cls.corr_df.iloc[i, :i]
    ...
    @classmethod
    def find_low_corr_combination(cls, init_factor, threshold=0.6, combination_num=3, max_num_per_iter=None,
        ...
        while flg:
            ...
            for i in range(len(combination_lis)):
                ...
                if greater_is_better:
                    candidate_ser = (cls.corr_df.iloc[:, current_combination] > threshold).sum(axis=1)
