...
class _Program(object):
    ...
    def build_program(self, random_state):
        ...
        if len(self.init_function_set) != 0:
            ...
            if random_state.randint(1, round(1/p)+1) == 1:
                ...
                function = deepcopy(self.init_function_set[function])
            else:
                function = random_state.randint(len(self.function_set))
                function = deepcopy(self.function_set[function])
                if function.isRandom:
                    if isinstance(function.RandRange,tuple):
        ...
        while terminal_stack:
            ...
            if (depth < max_depth) and (method == 'full' or
                ...
                function = random_state.randint(len(self.function_set))
                function = deepcopy(self.function_set[function])
                if function.need_param is None:
                    if function.isRandom:
                        if isinstance(function.RandRange,tuple):
                            current_window = random_state.randint(function.RandRange[0], function.RandRange[1])
                        else:
                ...
                # new add, treat fixed param function as a terminal
                else:
                    if function.isRandom:
                        if isinstance(function.RandRange,tuple):
                            current_window = random_state.randint(function.RandRange[0], function.RandRange[1])
                    ...
                    if terminal_stack[-1]>0:
                        if function in terminal_value_stack:
                            program.pop()
                            terminal_stack[-1] += 1
                        else:
    ...
    def __str__(self):
        ...
        for i, node in enumerate(self.program):
            if isinstance(node, _Function) and node.need_param is None:
                # if node.isRandom:
                ...
                if isinstance(node, _Function):
                    output += node.name + '('
                    for param in node.need_param:
                        output += param + ','
                    output = output[:-1]
                    if node.isRandom:
                        output += ',' + str(node.baseConst)
                    output += ')'
    ...
    def _depth(self):
        ...
        depth = 1
        for node in self.program:
            if isinstance(node, _Function) and node.need_param is None: # regard fixed param function as a terminal rather than functions, don't increase the depth
                terminals.append(node.arity)
    ...
    def _get_name_map(self, X):
        ...
        all_cols = X.columns
        # all_cols = all_cols[1:]
        all_cols = [col for col in all_cols if col != "col"]
        col_dictionary = {}
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                if (function.need_param is not None) and not (function.name.startswith('alpha_pool')):
                    terminals = [X[:, list(self.feature_names).index(param), :] for param in function.need_param]
                    # print(0, terminals)
                # alpha_pool
    ...
    def get_all_indices_df(self, X=pd.DataFrame(), n_samples=None, max_samples=None,
        ...
        sampled_df = X.groupby('col').apply(lambda x: x.sample(frac=0.9))
    ...
    def raw_fitness_3D(self, X, y, sample_weight, rolling_window_1, rolling_window_2, fee):
        """Evaluate the raw fitness of the program according to X, y.
        ...
        if self.transformer:
            y_pred = self.transformer(y_pred)
        raw_fitness = self.metric(y, y_pred, sample_weight, rolling_window_1, rolling_window_2, fee)
