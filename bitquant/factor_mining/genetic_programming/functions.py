...
def _cal_rolling_ic(y_pred, y, rolling_window):
    with np.errstate(divide='ignore', invalid='ignore'):
        no_future_beta = []
...
def _alpha_pool_2(x1, x2, close):
    try:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            x1 = _cal_rolling_ic(x1, y, rolling_window_2)
            x2 = _cal_rolling_ic(x2, y, rolling_window_2)
            x = x1 + x2
            return x
    except:
        return np.full(x1.shape, np.nan)
...
def _alpha_pool_3(x1, x2, x3, close):
    try:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            x1 = _cal_rolling_ic(x1, y, rolling_window_2)
            x2 = _cal_rolling_ic(x2, y, rolling_window_2)
            x3 = _cal_rolling_ic(x3, y, rolling_window_2)
            x = x1 + x2 + x3
            return x
    except:
        return np.full(x1.shape, np.nan)
